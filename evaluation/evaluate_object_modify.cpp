...
enum CLASSES{VEHICLE=0, PEDESTRIAN=1, CYCLIST=2};
...
void initGlobals () {
  ...
  CLASS_NAMES_CAP.push_back("Vehicle");
...
vector<tDetection> loadDetections(string file_name, bool &compute_aos,
  ...
  // std::cout <<"detections" <<file_name << std::endl;
...
vector<tGroundtruth> loadGroundtruth(string file_name,bool &success) {
  ...
  // std::cout << "file_name for ground truth" << file_name << std::endl;
...
tPrData computeStatistics(CLASSES current_class, const vector<tGroundtruth> &gt,
        ...
        METRIC metric, bool compute_aos=false, double thresh=-0.5, bool debug=false){
...
bool eval(Mail* mail){
  ...
  string gt_dir         = "../../dataset/gt_label";
  string result_dir     = "../../dataset/generate_label";
  ...
  for (int32_t i=0; i<N_TESTIMAGES; i++) {
    ...
    vector<tGroundtruth> gt   = loadGroundtruth(gt_dir + "/" + file_name+".txt",gt_success);
    vector<tDetection>   det  = loadDetections(result_dir + "/" + file_name+".txt",
...
int32_t main (int32_t argc,char *argv[]) {
  ...
  // if (argc!=2 && argc!=4) {
  //   cout << "Usage: ./eval_detection" << endl;
  //   return 1;
  // }
